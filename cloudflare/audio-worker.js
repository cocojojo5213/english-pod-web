...
export default {
  async fetch(request, env) {
    ...
    const id = new URL(request.url).pathname.match(/^\/media\/([a-z]+-\d{2})$/)?.[1];
    const size = JSON.parse(env.COURSES)[id];
    if (!size) return new Response(null,{status:404});
    let range;
    try { range = request.method === 'HEAD' ? null : parseRange(request.headers.get('Range'),size); }
    catch { return new Response(null,{status:416,headers:{'Content-Range':`bytes */${size}`}}); }
    const headers = new Headers({'Content-Type':'audio/mp4','Accept-Ranges':'bytes','Cache-Control':'private, max-age=86400','Content-Length':String(range?.length ?? size),'X-Audio-Source':'cloudflare-r2','X-Content-Type-Options':'nosniff'});
